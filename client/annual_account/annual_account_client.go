// Code generated by go-swagger; DO NOT EDIT.

package annual_account

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"

	"github.com/go-openapi/runtime"
	"github.com/go-openapi/strfmt"
)

// New creates a new annual account API client.
func New(transport runtime.ClientTransport, formats strfmt.Registry) ClientService {
	return &Client{transport: transport, formats: formats}
}

/*
Client for annual account API
*/
type Client struct {
	transport runtime.ClientTransport
	formats   strfmt.Registry
}

// ClientService is the interface for Client methods
type ClientService interface {
	LedgerAnnualAccountGet(params *LedgerAnnualAccountGetParams, authInfo runtime.ClientAuthInfoWriter) (*LedgerAnnualAccountGetOK, error)

	LedgerAnnualAccountSearch(params *LedgerAnnualAccountSearchParams, authInfo runtime.ClientAuthInfoWriter) (*LedgerAnnualAccountSearchOK, error)

	SetTransport(transport runtime.ClientTransport)
}

/*
  LedgerAnnualAccountGet gets annual account by ID
*/
func (a *Client) LedgerAnnualAccountGet(params *LedgerAnnualAccountGetParams, authInfo runtime.ClientAuthInfoWriter) (*LedgerAnnualAccountGetOK, error) {
	// TODO: Validate the params before sending
	if params == nil {
		params = NewLedgerAnnualAccountGetParams()
	}

	result, err := a.transport.Submit(&runtime.ClientOperation{
		ID:                 "LedgerAnnualAccount_get",
		Method:             "GET",
		PathPattern:        "/ledger/annualAccount/{id}",
		ProducesMediaTypes: []string{"application/json"},
		ConsumesMediaTypes: []string{"application/json"},
		Schemes:            []string{"https"},
		Params:             params,
		Reader:             &LedgerAnnualAccountGetReader{formats: a.formats},
		AuthInfo:           authInfo,
		Context:            params.Context,
		Client:             params.HTTPClient,
	})
	if err != nil {
		return nil, err
	}
	success, ok := result.(*LedgerAnnualAccountGetOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for LedgerAnnualAccount_get: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}

/*
  LedgerAnnualAccountSearch finds annual accounts corresponding with sent data
*/
func (a *Client) LedgerAnnualAccountSearch(params *LedgerAnnualAccountSearchParams, authInfo runtime.ClientAuthInfoWriter) (*LedgerAnnualAccountSearchOK, error) {
	// TODO: Validate the params before sending
	if params == nil {
		params = NewLedgerAnnualAccountSearchParams()
	}

	result, err := a.transport.Submit(&runtime.ClientOperation{
		ID:                 "LedgerAnnualAccount_search",
		Method:             "GET",
		PathPattern:        "/ledger/annualAccount",
		ProducesMediaTypes: []string{"application/json"},
		ConsumesMediaTypes: []string{"application/json"},
		Schemes:            []string{"https"},
		Params:             params,
		Reader:             &LedgerAnnualAccountSearchReader{formats: a.formats},
		AuthInfo:           authInfo,
		Context:            params.Context,
		Client:             params.HTTPClient,
	})
	if err != nil {
		return nil, err
	}
	success, ok := result.(*LedgerAnnualAccountSearchOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for LedgerAnnualAccount_search: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}

// SetTransport changes the transport on the client
func (a *Client) SetTransport(transport runtime.ClientTransport) {
	a.transport = transport
}
