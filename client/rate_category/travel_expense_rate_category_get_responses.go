// Code generated by go-swagger; DO NOT EDIT.

package rate_category

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"
	"github.com/go-openapi/strfmt"

	"github.com/bjerkio/tripletex-go/v2/models"
)

// TravelExpenseRateCategoryGetReader is a Reader for the TravelExpenseRateCategoryGet structure.
type TravelExpenseRateCategoryGetReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *TravelExpenseRateCategoryGetReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {
	case 200:
		result := NewTravelExpenseRateCategoryGetOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil

	default:
		return nil, runtime.NewAPIError("response status code does not match any response statuses defined for this endpoint in the swagger spec", response, response.Code())
	}
}

// NewTravelExpenseRateCategoryGetOK creates a TravelExpenseRateCategoryGetOK with default headers values
func NewTravelExpenseRateCategoryGetOK() *TravelExpenseRateCategoryGetOK {
	return &TravelExpenseRateCategoryGetOK{}
}

/*TravelExpenseRateCategoryGetOK handles this case with default header values.

successful operation
*/
type TravelExpenseRateCategoryGetOK struct {
	Payload *models.ResponseWrapperTravelExpenseRateCategory
}

func (o *TravelExpenseRateCategoryGetOK) Error() string {
	return fmt.Sprintf("[GET /travelExpense/rateCategory/{id}][%d] travelExpenseRateCategoryGetOK  %+v", 200, o.Payload)
}

func (o *TravelExpenseRateCategoryGetOK) GetPayload() *models.ResponseWrapperTravelExpenseRateCategory {
	return o.Payload
}

func (o *TravelExpenseRateCategoryGetOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.ResponseWrapperTravelExpenseRateCategory)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
