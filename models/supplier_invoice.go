...
func (m *SupplierInvoice) Validate(formats strfmt.Registry) error {
	...
	if err := m.validateApprovalListElements(formats); err != nil {
		res = append(res, err)
	}
	...
	if err := m.validateOriginalInvoiceDocumentID(formats); err != nil {
		res = append(res, err)
	}
...
func (m *SupplierInvoice) validateApprovalListElements(formats strfmt.Registry) error {

	if swag.IsZero(m.ApprovalListElements) { // not required
		return nil
	}

	for i := 0; i < len(m.ApprovalListElements); i++ {
		if swag.IsZero(m.ApprovalListElements[i]) { // not required
			continue
		}

		if m.ApprovalListElements[i] != nil {
			if err := m.ApprovalListElements[i].Validate(formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					return ve.ValidateName("approvalListElements" + "." + strconv.Itoa(i))
				}
				return err
			}
		}
...
func (m *SupplierInvoice) validateOriginalInvoiceDocumentID(formats strfmt.Registry) error {

	if swag.IsZero(m.OriginalInvoiceDocumentID) { // not required
		return nil
	}

	if err := validate.MinimumInt("originalInvoiceDocumentId", "body", int64(m.OriginalInvoiceDocumentID), 0, false); err != nil {
		return err
	}
