...
var postingTypeTypePropEnum []interface{}

func init() {
	var res []string
	if err := json.Unmarshal([]byte(`["INCOMING_PAYMENT","INCOMING_PAYMENT_OPPOSITE","INVOICE_EXPENSE"]`), &res); err != nil {
		panic(err)
	}
	for _, v := range res {
		postingTypeTypePropEnum = append(postingTypeTypePropEnum, v)
	}
}
...
// prop value enum
func (m *Posting) validateTypeEnum(path, location string, value string) error {
	if err := validate.EnumCase(path, location, value, postingTypeTypePropEnum, true); err != nil {
		return err
	}
	return nil
}

func (m *Posting) validateType(formats strfmt.Registry) error {
