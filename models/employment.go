...
func (m *Employment) Validate(formats strfmt.Registry) error {
	...
	if err := m.validateEmploymentEndReason(formats); err != nil {
		res = append(res, err)
	}
...
var employmentTypeEmploymentEndReasonPropEnum []interface{}

func init() {
	var res []string
	if err := json.Unmarshal([]byte(`["EMPLOYMENT_END_EXPIRED","EMPLOYMENT_END_EMPLOYEE","EMPLOYMENT_END_EMPLOYER","EMPLOYMENT_END_WRONGLY_REPORTED","EMPLOYMENT_END_SYSTEM_OR_ACCOUNTANT_CHANGE","EMPLOYMENT_END_INTERNAL_CHANGE"]`), &res); err != nil {
		panic(err)
	}
	for _, v := range res {
		employmentTypeEmploymentEndReasonPropEnum = append(employmentTypeEmploymentEndReasonPropEnum, v)
	}
}
...
// prop value enum
func (m *Employment) validateEmploymentEndReasonEnum(path, location string, value string) error {
	if err := validate.EnumCase(path, location, value, employmentTypeEmploymentEndReasonPropEnum, true); err != nil {
		return err
	}
	return nil
}

func (m *Employment) validateEmploymentEndReason(formats strfmt.Registry) error {

	if swag.IsZero(m.EmploymentEndReason) { // not required
		return nil
	}

	// value enum
	if err := m.validateEmploymentEndReasonEnum("employmentEndReason", "body", m.EmploymentEndReason); err != nil {
		return err
	}
